...
export async function GET(req: NextRequest) {
  ...
  if (type === "cod_by_wilaya") {
    const { data, error } = await supabase.rpc("cod_stats_by_wilaya", { p_days: days });
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ stats: data ?? [] });
  }
  ...
  if (type === "revenue_by_month") {
    ...
    const { data, error } = await supabase.rpc("revenue_by_month", { p_months: months });
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ stats: data ?? [] });
  }
  ...
  if (type === "delivery") {
    ...
    for (const d of data ?? []) {
      const p   = extractRelation<{ name: string; code: string }>(d.delivery_partners);
