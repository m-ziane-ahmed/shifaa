...
export async function GET(req: NextRequest) {
  ...
  if (type === "partners") {
    const { data, error } = await supabase.from("delivery_partners").select("*").order("priority");
  ...
  if (type === "active") {
    const { data, count, error } = await supabase
      .from("deliveries")
      .select(`*, delivery_partners(name, code, tracking_url, logo_url), orders(id, total, wilaya, guest_name, guest_phone, payment)`, { count: "exact" })
  ...
  if (type === "stats") {
    const { data, error } = await supabase
      .from("deliveries")
      .select("status, partner_id, delivery_partners(name, code)");
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });

    const stats: Record<string, { name: string; total: number; delivered: number; failed: number; in_transit: number }> = {};
    for (const d of data ?? []) {
      const partner = extractRelation<{ name: string; code: string }>(d.delivery_partners);
